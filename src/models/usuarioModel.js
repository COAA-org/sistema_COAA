...
function cadastrarEmpresa(cnpj, desc, nome, telefone, email, senha) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", nome, email, senha);
    //alterar aqui para os parâmetros da tabela 
    // Insira exatamente a query do banco aqui, lembrando da nomenclatura exata nos valores
    //  e na ordem de inserção dos dados.
    var instrucao = `
        INSERT INTO Empresa (cnpj, razaoEmpresa, nomeEmpresa, telefone, email, senha) VALUES ('${cnpj}', '${desc}', '${nome}', '${telefone}', '${email}', '${senha}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
function cadastrarFabrica(nome, telefone) {
    ...
    ///////////////////////////////////
    var instrucao = `
        INSERT INTO Fabrica (nomeFabrica, telefone) VALUES ('${nome}', '${telefone}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
